...
class Enemy(pygame.sprite.Sprite):
    ...
    def update(self, pipes, blocks, rlocks):
        ...
        if self.live:
            ...
            # Проверка столкновений с твердыми блоками
            for rlock in rlocks:
...
class Me(pygame.sprite.Sprite):
    def __init__(self, x, y, file):
        ...
        self.numa = False  
        self.large = False  # Изначально маленький
        self.last_move_time = None
        self.dead_time = 0 
        self.timer_start = None
        ...
        self.timer_running = False
        self.font = pygame.font.Font(None, 36)
        self.immortality = False  # Флаг бессмертия
        self.he = False
        self.elapsed_time = None
        self.victory = False
        self.on_flag = False  # Флаг, который отслеживает, находит ли Марио на флагштоке
        self.flag_descend_speed = 0.2  # Начальная медленная скорость спуска
        self.is_descending = False  # Флаг, показывающий, что Марио спускается
        self.descend_acceleration = 0.05  # Очень медленное увеличение скорости спуска
        self.collipol = True
        self.just_got_mushroom = False  # Новый флаг
        self.state = "normal"  # Добавляем состояние Марио
        self.run_to_castle = False  # Флаг для бега к замку
        self.run_speed = 2  # Скорость бега к замку
        self.ga = False  # Флаг для отслеживания касания флага
        self.touched_ground_after_flag = False  # Флаг для отслеживания касания пола после флага
        ...
        if self.state == "normal":
            ...
            if self.ouch:
                ...
                if self.immortality:
                    ...
                    if self.timer_running:
                        self.elapsed_time = pygame.time.get_ticks() - self.timer_start
                        self.remaining_time = max(0, self.timer_duration - self.elapsed_time)  # Оставшееся время
                        if self.remaining_time == 0:
    ...
    def descend_from_flag(self):
        if self.rect.y < height - 65:  # Спускаем, пока он не достигнет земли
            self.rect.y += self.flag_descend_speed  # Спускаем с флагштока
            self.flag_descend_speed += 0.005  # Постепенно увеличиваем скорость для плавности
            if self.flag_descend_speed > 1:  # Ограничиваем максимальную скорость
    ...
    def run_to_castle_animation(self):
        if self.rect.x < tower.rect.x:  # Бежим к замку
            self.speed = self.run_speed  # Устанавливаем скорость бега
            self.rect.x += self.speed
            self.Go = True
            self.Left = False
            self.Right = True
            self.Stand = False
            self.animate()  # Обновляем анимацию бега
        else:
            self.remove()  # Исчезаем при достижении замка
...
# Функция для случайного спавна врагов
def spawn_enemies(num_enemies, min_x, max_x, y, pipes):
    enemies = pygame.sprite.Group()
    for _ in range(num_enemies):
        valid_spawn = False
        while not valid_spawn:
            x = random.randint(min_x, max_x)  # Случайная позиция по x
            new_enemy = Enemy(x, y, 'mwalk/Goomba_Walk1.png')
            
            # Проверяем на столкновение с трубами
            valid_spawn = True
            for pipe in pipes:
                if new_enemy.rect.colliderect(pipe.rect):  # Если враг перекрывает трубу
                    valid_spawn = False
                    break  # Если перекрывает трубу, пробуем снова
            
        enemies.add(new_enemy)  # Добавляем валидного врага в группу
    return enemies
gran = pygame.Rect(c_x, c_y, 10, 200)
flags_group = pygame.sprite.Group()  # Создаем группу для флага
flag = Flag(f_x, f_y, 18, 180, 'Mario Game Assets/Mario Game Assets/FlagPole.png')
flags_group.add(flag)  # Добавляем флаг в группу
##bg = pygame.image.load("bg-1-1-a.png")
...
while lol:
    ...
    if me.large:
        for block in blocks:
            if pygame.sprite.collide_rect(me, block):
                # Корректируем коллизию с верхом блока, если Марио большой
                if me.rect.bottom >= block.rect.top and me.rect.top < block.rect.bottom-40:
                    if me.dy > 0:  # Если Марио падает
                        me.rect.bottom = block.rect.top-40  # Марио не должен пройти внутрь блока
                        me.dy = 0
                        me.onGround = True
                        me.Jump = False
    ...
    for enemy in enemies:
        if pygame.sprite.collide_rect(me, enemy):
            if me.dy > 0 and me.rect.bottom > enemy.rect.top and me.rect.bottom - me.dy <= enemy.rect.top and me.Jump and me.live: 
                ...
                if me.large and not me.just_got_mushroom:  # Если Марио большой и не только что получил гриб
                    # Марио становится маленьким и получает бессмертие на 5 секунд
                    me.large = False
                    me.immortality = True
                    enemy.live=False
                elif not me.large:  # Если Марио маленький — он умирает
                    me.live = False
        ...
        if me.just_got_mushroom:
            me.just_got_mushroom = False  # Сбрасываем флаг после обработки столкновений
        for rlock in rlocks:
    ...
    if pygame.sprite.collide_rect(me, flag):  
        me.state = "on_flag"  # Марио начинает спускаться по флагу
        me.ga=True
        win_sound.play()
    ...
    if me.live: 
        if keys[pygame.K_d] and coll ==False:
            ...
            if me.rect.x > 450:
                me.rect.x = 450
                c_x -= me.speed + 1.5
                f_x -= me.speed + 1.2  # Смещаем флаг в том же направлении, что и игрок
                c2_xx -= me.speed + 1.5
                for pipe in pipes:
    ...
    ##screen.blit(bg, (c_x, c_y))
    screen.blit(bg_up, (c_xx2, c_yy2))
    bg_pol_group.draw(screen)  # Отрисовываем флаг
    pygame.draw.rect(screen, RED, gran)
    flags_group.draw(screen)  # Отрисовываем флаг
    blocks.draw(screen)
    rlocks.draw(screen)
